...
class Solution {
    public static ArrayList<ArrayList<Integer>> findPairsWithGivenSum(int t, Node head) {
        // code here
       ...
       ArrayList<ArrayList<Integer>> ls=new ArrayList<>();
        while(head!=null){
            ...
            if(hs.contains(t-x)){
                if(x<(t-x))
                ls.add(new ArrayList<>(Arrays.asList(x,t-x)));
                else
                ls.add(new ArrayList<>(Arrays.asList(t-x,x)));
            }
            else
            hs.add(x);
            head=head.next;
